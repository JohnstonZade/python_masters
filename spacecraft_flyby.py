...
def flyby(output_dir, a, do_last=1, do_rand_start=1, n=None, l_start=None):
    if do_last:
        n = diag.get_maxn(output_dir) - 1
    else:
        assert n is not None, 'n must be an integer!'
    
    # assuming always from_array
    data = diag.load_data(output_dir, n, prob='from_array')
    Ns, Ls = get_grid_info(data, a[n])
    ...
    By = a[n]*pad_array(data['Bcc2'])
    Bz = a[n]*pad_array(data['Bcc3'])
    ...
    N_y = Ns[1]
    total_length = a[n]*N_y**2 if N_y <= 256 else a[n]*N_y
    dl = yg[1] - yg[0]
